...
use std::thread; // includes a large number of function such as 
...
fn main() {
    ...
    let xs2 = xs.clone(); // cloning xs so it can be used with the thread

    // create 2 threads that run concurrently and each of which uses map_data() function to process one of the two partitions
    let thread1 = thread::spawn(move || map_data(&xs[0]));
    let thread2 = thread::spawn(move || map_data(&xs2[1]));

    // join to end thread 
    let join_end1 = thread1.join().unwrap(); // snake case name
    let join_end2 = thread2.join().unwrap(); // snake case name
    
    // intermediate sum
    intermediate_sums.push(join_end1);
    intermediate_sums.push(join_end2);
   ...
   // intermediate_sums.push(map_data(&xs[1]));
    ...
    // 1. Calls partition_data to partition the data into equal partitions
    let create_participant = partition_data(num_partitions, &v); // snake case 
    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&create_participant);
    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    let mut intermediate_sums2: Vec<usize> = Vec::new();
    for thread_loop in 0..num_partitions{ // snake case name
       
        let clone_each_vector = create_participant.clone();

        let new_thread = thread::spawn(move || map_data(&clone_each_vector[thread_loop])); //snake case name
        let return_thread = new_thread.join().unwrap();// snake case name

        intermediate_sums2.push(return_thread);
    }
    // 4. Collects the intermediate sums from all the threads
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut start_no_participants = 0;
    let mut end_no_participants = partition_size;

    // loop for going through the number of paritions 
    for cycle_num_participants in 0..num_partitions{
        let mut i : Vec<usize> = Vec::new();
        for j in start_no_participants..end_no_participants{
            i.push(v[j]);
        }
        xs.push(i);
        
        if add_one_value > 0{ // uneven participants size
            xs[cycle_num_participants].push(v[end_no_participants]);
            start_no_participants = start_no_participants + partition_size + 1;
            end_no_participants = end_no_participants + partition_size + 1;
            add_one_value -=1; 
        }
        else{ // adds the reset of start number of participants and the end number of participants 
            start_no_participants = start_no_participants + partition_size;
            end_no_participants = end_no_participants + partition_size;
        }
    }
    xs // returning xs as the final vector
}
